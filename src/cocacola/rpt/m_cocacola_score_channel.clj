...
(ns cocacola.rpt.m-cocacola-score_channel
...
(defn score-sliding  [[start-dt end-dt]]
  (<- [?dw-dt ?bg ?bottler ?channel !code ?item ?fact ?value ?bottler_sort ?bg_sort ?channel_sort ?kpi_sort ?max-value !pp-value !vs-pp-value !last-dec-value !vs-last-dec-value !last-year-same-month-value !vs-last-year-same-month-value]
      ...
      #_((c/each vs-value) ?value !pp-value :> !vs-pp-value)
...
#_(?<- (stdout)
     [?dw-dt ?bg ?bottler ?channel !code ?item ?fact ?value ?bottler_sort ?bg_sort ?channel_sort ?kpi_sort ?max-value !pp-value !vs-pp-value !last-dec-value !vs-last-dec-value !last-year-same-month-value !vs-last-year-same-month-value]
     ((score-sliding ["2015-01-01" "2015-01-01"]) :> ?dw-dt ?bg ?bottler ?channel !code ?item ?fact ?value ?bottler_sort ?bg_sort ?channel_sort ?kpi_sort ?max-value !pp-value !vs-pp-value !last-dec-value !vs-last-dec-value !last-year-same-month-value !vs-last-year-same-month-value))



(defn score-channel-report [[start-dt end-dt]]
  (<- [?dw-dt ?project ?category ?report ?selector ?selector-desc ?dimension-metrics]
      ((score-sliding [start-dt end-dt]) :> ?dw-dt ?bg ?bottlers ?channel !code ?item ?fact ?value ?bottler_sort ?bg_sort ?channel_sort ?kpi_sort ?max-value !pp-value !vs-pp-value !last-dec-value !vs-last-dec-value !last-year-same-month-value !vs-last-year-same-month-value)
      (not !code)
      ((mapfn [it] (let [kpi (first (str/split it #"-"))]
                 (if (= kpi "产品铺货率") "产品铺货" kpi))) ?item :> ?kpi)
      (re-find #"^全体$" ?kpi)
      (not= ?channel "Total / 所有渠道")
      ((mapfn [bottler] (and (when-not (re-find #"\S*区域$" bottler) bottler)
                             (when-not (re-find #"\S*辽宁$" bottler) bottler)
                             (when-not (re-find #"Tier\d" bottler) bottler))) ?bottlers :> ?bottler)
      (identity ["cocacola" "score" "channel" ""] :> ?project ?category ?report ?selector-desc)
      ((vars->pair [:period :bg :bottler]) ?dw-dt ?bg_sort ?bottler_sort :> ?selector-edn)
      ((tr-dimension-metrics [:channel] [:score :pp_score :vs_pp_score :last_dec_score :vs_last_dec_score :last_year_same_month_score :vs_last_year_same_month_score]) ?channel ?value !pp-value !vs-pp-value !last-dec-value !vs-last-dec-value !last-year-same-month-value !vs-last-year-same-month-value :> ?dimension-metrics-edn)
      ((c/each pair-edn->json) ?selector-edn ?dimension-metrics-edn :> ?selector ?dimension-metrics)))

#_(?<- (stdout)
        [?dw-dt ?project ?category ?report ?selector ?selector-desc ?dimension-metrics]
        ((score-channel-report ["2015-01-01" "2015-01-01"]) :> ?dw-dt ?project ?category ?report ?selector ?selector-desc ?dimension-metrics))


(defn -main []
  (def dt-rng (report->next-dt report-tap-out "channel"))
  (prn {:dt-rng dt-rng :rpt "channel"} "running...")
  (try (?- report-tap-out (score-channel-report dt-rng)) (catch Exception _))
  (prn {:dt-rng dt-rng} "done!"))

#_(comment
  (??- (score-channel-report (report->next-dt report-tap-out "channel")))
  (map println (??- (score-channel-report ["2015-01-01" "2016-01-01"])))
  (?<- (stdout)
       [?dw-dt ?project ?category ?report ?selector ?selector-desc ?dimension-metrics]
       ((score-channel-report ["1970-01-01" "2015-12-31"]) :> ?dw-dt ?project ?category ?report ?selector ?selector-desc ?dimension-metrics))
  )
