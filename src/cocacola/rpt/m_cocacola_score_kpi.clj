...
(ns cocacola.rpt.m-cocacola-score_kpi
...
(defn score-kpi-report [[start-dt end-dt]]
  (<- [?dw-dt ?project ?category ?report ?selector ?selector-desc ?dimension-metrics]
      ...
      (identity ["cocacola" "score" "kpi" ""] :> ?project ?category ?report ?selector-desc)
      ((vars->pair [:period :bg :bottler]) ?dw-dt ?bg_sort ?bottler_sort :> ?selector-edn)
      ((tr-dimension-metrics [:kpi] [:score :pp_score :vs_pp_score :last_dec_score :vs_last_dec_score :last_year_same_month_score :vs_last_year_same_month_score]) ?kpi ?value !pp-value !vs-pp-value  !last-dec-value !vs-last-dec-value !last-year-same-month-value !vs-last-year-same-month-value :> ?dimension-metrics-edn)
      ((c/each pair-edn->json) ?selector-edn ?dimension-metrics-edn :> ?selector ?dimension-metrics)))

#_ (?<- (stdout)
        [?count]
        ((score-kpi-report ["2015-01-01" "2016-12-31"]) :> ?dw-dt ?project ?category ?report ?selector ?selector-desc ?dimension-metrics)
        (c/count ?count))

(defn -main []
  (def dt-rng (report->next-dt report-tap-out "kpi"))
  (prn {:dt-rng dt-rng :rpt "kpi"} "running...")
  (try (?- report-tap-out (score-kpi-report dt-rng)) (catch Exception _))
