...
(ns cocacola.rpt.m-cocacola-gt-score-deep-dive-without-ko
...
(def score-tap-in (pg-tap "dw" "model.d_cocacola_gt_score_deep_dive_without_ko" ["dw-dt" "period" "mbd" "bg" "bottler" "channel" "code" "item" "fact" "value" "abbrevation" "bg_sort" "p_item" "item_weight" "item_order" "total_score"]))
...
(def m-cocacola-gt-score-deep-dive-without-ko
    (<- [?dw-dt ?project ?category ?report ?selector ?selector-desc ?dimension-metrics]
        ...
        (identity ["9999-12-31" "cocacola" "score" "gt_score_deep_dive_without_ko" ""] :> ?dw-dt ?project ?category ?report ?selector-desc)
...
(defn -main []
  ...
  (detele-report! report-tap-out "gt_score_deep_dive_without_ko" dt)
  (def dt-rng (report->next-dt report-tap-out "gt_score_deep_dive_without_ko"))
  (prn {:dt-rng dt-rng :rpt "gt_score_deep_dive_without_ko"} "running...")
  (try (?- report-tap-out m-cocacola-gt-score-deep-dive-without-ko) (catch Exception _))
