...
(ns cocacola.rpt.m-cocacola-score_channel_bg
...
(defn score-channel_bg-report [[start-dt end-dt]]
  (<- [?dw-dt ?project ?category ?report ?selector ?selector-desc ?dimension-metrics]
      ...
      (identity ["cocacola" "score" "channel_bg" ""] :> ?project ?category ?report ?selector-desc)
      ((vars->pair [:period :bottler]) ?dw-dt ?bottler_sort :> ?selector-edn)
      ((tr-dimension-metrics [:channel :bottler] [:score :pp_score :vs_pp_score :last_dec_score :vs_last_dec_score :last_year_same_month_score :vs_last_year_same_month_score]) ?channel ?bottler_sort ?value !pp-value !vs-pp-value !last-dec-value !vs-last-dec-value !last-year-same-month-value !vs-last-year-same-month-value :> ?dimension-metrics-edn)
      ((c/each pair-edn->json) ?selector-edn ?dimension-metrics-edn :> ?selector ?dimension-metrics)))


#_ (?<- (stdout)
        [?dw-dt ?project ?category ?report ?selector ?selector-desc ?dimension-metrics]
        ((score-channel_bg-report ["2015-01-01" "2016-12-31"]) :> ?dw-dt ?project ?category ?report ?selector ?selector-desc ?dimension-metrics)
        )


(defn -main []
  (def dt-rng (report->next-dt report-tap-out "channel_bg"))
  (prn {:dt-rng dt-rng :rpt "channel_bg"} "running...")
  (try (?- report-tap-out (score-channel_bg-report dt-rng)) (catch Exception _))
