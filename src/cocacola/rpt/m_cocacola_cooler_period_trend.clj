...
(ns cocacola.rpt.m-cocacola-cooler_period_trend
...
(defn cooler_period_trend-report [[start-dt end-dt]]
  (<- [?dw-dt ?project ?category ?report ?selector ?selector-desc ?dimension-metrics]
      ...
      (= ?brand "KO")
      (identity ["cocacola" "score" "cooler_period_trend" ""] :> ?project ?category ?report ?selector-desc)
      ((vars->pair [:channel :kpi :cooler_type :bottler]) ?channel_sort "COOLER" ?type ?bottler_sort :> ?selector-edn)
...
#_ (?<- (stdout)
        [?count]
        ((cooler_period_trend-report ["2015-01-01" "2016-12-31"]) :> ?dw-dt ?project ?category ?report ?selector ?selector-desc ?dimension-metrics)
        (c/count ?count))

(defn -main []
  (def dt ["9999-12-31" "9999-12-31"])
  (detele-report! report-tap-out "cooler_period_trend" dt)
  (def dt-rng (report->next-dt report-tap-out "cooler_period_trend"))
  (prn {:dt-rng dt-rng :rpt "cooler_period_trend"} "running...")
  (try (?- report-tap-out (cooler_period_trend-report dt-rng)) (catch Exception _))
