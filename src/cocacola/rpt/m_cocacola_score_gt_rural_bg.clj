...
(ns cocacola.rpt.m-cocacola-score_gt_rural_bg
...
(defn score-gt_rural_bg-report [[start-dt end-dt]]
  (<- [?dw-dt ?project ?category ?report ?selector ?selector-desc ?dimension-metrics]
      ...
      (identity ["cocacola" "score" "GT_Rural_Bg" ""] :> ?project ?category ?report ?selector-desc)
      ((vars->pair [:period :button]) ?dw-dt "test" :> ?selector-edn)
      ((tr-dimension-metrics [:bg] [:score :pp_score :vs_pp_score :last_dec_score :vs_last_dec_score :last_year_same_month_score :vs_last_year_same_month_score]) ?bg ?value !pp-value !vs-pp-value !last-dec-value !vs-last-dec-value !last-year-same-month-value !vs-last-year-same-month-value :> ?dimension-metrics-edn)
      ((c/each pair-edn->json) ?selector-edn ?dimension-metrics-edn :> ?selector ?dimension-metrics)))

#_(?<- (stdout)
        [?dw-dt ?project ?category ?report ?selector ?selector-desc ?dimension-metrics]
        ((score-gt_rural_bg-report ["2015-01-01" "2016-12-31"]) :> ?dw-dt ?project ?category ?report ?selector ?selector-desc ?dimension-metrics))

(defn -main []
  (def dt-rng (report->next-dt report-tap-out "GT_Rural_Bg"))
  (prn {:dt-rng dt-rng :rpt "GT_Rural_Bg"} "running...")
  (try (?- report-tap-out (score-gt_rural_bg-report dt-rng)) (catch Exception _))
