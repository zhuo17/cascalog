...
(ns cocacola.rpt.m-cocacola-sku6_bg
...
(defn sku6_bg-report [[start-dt end-dt]]
  (<- [?dw-dt ?project ?category ?report ?selector ?selector-desc ?dimension-metrics]
      ...
      (= ?bottler "Total")
      (= ?sku_type "6 SKU")
      (identity ["cocacola" "score" "sku6_bg" ""] :> ?project ?category ?report ?selector-desc)
...
#_(?<- (stdout)
     [?dw-dt ?project ?category ?report ?selector ?selector-desc ?dimension-metrics]
     ((sku6_bg-report ["2015-01-01" "2016-01-01"]) :> ?dw-dt ?project ?category ?report ?selector ?selector-desc ?dimension-metrics))

(defn -main []
  (def dt-rng (report->next-dt report-tap-out "sku6_bg"))
  (prn {:dt-rng dt-rng :rpt "sku6_bg"} "running...")
  (try (?- report-tap-out (sku6_bg-report dt-rng)) (catch Exception _))
