...
(ns cocacola.rpt.m-cocacola-score-tier
...
(def score-tap-in (pg-tap "dw" "model.d_cocacola_score_tier" ["dw-dt" "period" "mbd" "bg" "bottler" "channel" "code" "item" "fact" "value" "abbrevation" "bg_sort" "channel_sort" "kpi_sort"]))
...
(def score-tier-report 
  (<- [?dw-dt ?project ?category ?report ?selector ?selector-desc ?dimension-metrics]
      ...
      (not !code)
      (identity ["cocacola" "score" "by_bottler_tier" ""] :> ?project ?category ?report ?selector-desc)
      ((vars->pair [:channel :facts]) ?channel_sort ?kpi_sort :> ?selector-edn)
...
(defn -main []
  ...
  (detele-report! report-tap-out "by_bottler_tier" dt)
  (def dt-rng (report->next-dt report-tap-out "by_bottler_tier"))
  (prn {:dt-rng dt-rng :rpt "by_bottler_tier"} "running...")
