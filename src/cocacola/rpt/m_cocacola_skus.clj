...
(ns cocacola.rpt.m-cocacola-skus
...
(defn skus-report [[start-dt end-dt]]
  (<- [?dw-dt ?project ?category ?report ?selector ?selector-desc ?dimension-metrics]
      ...
      (identity ["cocacola" "score" "skus" ""] :> ?project ?category ?report ?selector-desc)
      ((vars->pair [:period :bg :bottler :channel]) ?dw-dt ?bg_sort ?bottler_sort ?channel_sort :> ?selector-edn)
      ((tr-dimension-metrics [:sku_type] [:score :pp_score :vs_pp_score :last_dec_score :vs_last_dec_score :last_year_same_month_score :vs_last_year_same_month_score :abbr])  ?sku_type ?value !pp-value !vs-pp-value !last-dec-value !vs-last-dec-value !last-year-same-month-value !vs-last-year-same-month-value ?bottler_sort :> ?dimension-metrics-edn)
      ((c/each pair-edn->json) ?selector-edn ?dimension-metrics-edn :> ?selector ?dimension-metrics)))

#_ (?<- (stdout)
        [?dw-dt ?project ?category ?report ?selector ?selector-desc ?dimension-metrics]
        ((skus-report ["2015-01-01" "2016-01-01"]) :> ?dw-dt ?project ?category ?report ?selector ?selector-desc ?dimension-metrics))

(defn -main []
  (def dt-rng (report->next-dt report-tap-out "skus"))
  (prn {:dt-rng dt-rng :rpt "skus"} "running...")
  (try (?- report-tap-out (skus-report dt-rng)) (catch Exception _))
