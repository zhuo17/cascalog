...
(ns cocacola.rpt.m-cocacola-score_overall
...
(def score-dt-kv
  (<- [?bg_sort ?bottler_sort ?channel !code ?item ?fact ?dw-dt-kv]
...
(defn score-sliding  [[start-dt end-dt]]
  (<- [?dw-dt ?bg ?bottler ?channel !code ?item ?fact ?value ?max-value !pp-value !vs-pp-value !last-dec-value !vs-last-dec-value !last-year-same-month-value !vs-last-year-same-month-value]
      (score-dt-kv :> ?bg ?bottler ?channel !code ?item ?fact ?dw-dt-kv)
...
(defn score-overall-report [[start-dt end-dt]]
  (<- [?dw-dt ?project ?category ?report ?selector ?selector-desc ?dimension-metrics]
      ((score-sliding [start-dt end-dt]) :> ?dw-dt ?bg ?bottlers ?channel !code ?item ?fact ?value ?max-value !pp-value !vs-pp-value !last-dec-value !vs-last-dec-value !last-year-same-month-value !vs-last-year-same-month-value)
      (not !code)
      ...
      (identity ["cocacola" "score" "overall" ""] :> ?project ?category ?report ?selector-desc)
      ((vars->pair [:period :bg :bottler]) ?dw-dt ?bg ?bottler :> ?selector-edn)
      ((tr-dimension-metrics [] [:score :pp_score :vs_pp_score :last_dec_score :vs_last_dec_score :last_year_same_month_score :vs_last_year_same_month_score]) ?value !pp-value !vs-pp-value !last-dec-value !vs-last-dec-value !last-year-same-month-value !vs-last-year-same-month-value :> ?dimension-metrics-edn)
      ((c/each pair-edn->json) ?selector-edn ?dimension-metrics-edn :> ?selector ?dimension-metrics)))

#_ (?<- (stdout)
        [?count]
        ((score-overall-report ["2015-01-01" "2016-12-31"]) :> ?dw-dt ?project ?category ?report ?selector ?selector-desc ?dimension-metrics)
        (c/count ?count))


(defn -main []
  (def dt-rng (report->next-dt report-tap-out "overall"))
  (prn {:dt-rng dt-rng :rpt "overall"} "running...")
  (try (?- report-tap-out (score-overall-report dt-rng)) (catch Exception _))
