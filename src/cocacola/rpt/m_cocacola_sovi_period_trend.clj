...
(ns cocacola.rpt.m-cocacola-sovi_period_trend
...
(defn sovi_period_trend-report [[start-dt end-dt]]
  (<- [?dw-dt ?project ?category ?report ?selector ?selector-desc ?dimension-metrics]
      ...
      (= ?brand "KO")
      ((mapfn [juice_type sovi_type] (str juice_type "_" sovi_type)) ?juice_type ?sovi_type :> ?juice_sovi_type)
      (identity ["cocacola" "score" "sovi_period_trend" ""] :> ?project ?category ?report ?selector-desc)
      ((vars->pair [:channel :kpi :juice_sovi_type :bottler]) ?channel_sort "SOVI" ?juice_sovi_type ?bottler_sort :> ?selector-edn)
...
#_(?<- (stdout)
     [?dw-dt ?project ?category ?report ?selector ?selector-desc ?dimension-metrics]
     ((sovi_period_trend-report ["2014-01-01" "2016-12-31"]) ?dw-dt ?project ?category ?report ?selector ?selector-desc ?dimension-metrics))

(defn -main []
  (def dt ["9999-12-31" "9999-12-31"])
  (detele-report! report-tap-out "sovi_period_trend" dt)
  (def dt-rng (report->next-dt report-tap-out "sovi_period_trend"))
  (prn {:dt-rng dt-rng :rpt "sovi_period_trend"} "running...")
  (try (?- report-tap-out (sovi_period_trend-report dt-rng)) (catch Exception _))
